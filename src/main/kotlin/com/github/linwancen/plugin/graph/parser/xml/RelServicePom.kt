...
object RelServicePom {
    ...
    fun parsePom(
        ...
        for (it in psiFile.children) {
            ...
            dependencies(root, callSet, relData)
        }
    }
    ...
    private fun dependencies(root: XmlTag, callList: MutableSet<String>, relData: RelData) {
        root.findFirstSubTag("dependencies")?.let { tag ->
            val items = tag.findSubTags("dependency")
            if (items.isNotEmpty()) {
                for (item in items) {
                    val itemInfo = info(item) ?: continue
                    val sign = itemInfo["sign"] ?: continue
                    callList.add(sign)
                    // 注册依赖项作为节点，这样它们才能在图中显示
                    relData.regParentChild(itemInfo)
                }
            }
        }
    }
